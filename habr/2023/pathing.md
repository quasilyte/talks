...
Алгоритмы на сегодня: A-star и <abbr title="Greedy best-first search, не путать с breadth-first search">greedy BFS</abbr>. Оба отлично разобраны в статье [Introduction to the A* Algorithm](https://www.redblobgames.com/pathfinding/a-star/introduction.html).
...
Мне в моей задаче нужен поиск по <abbr title="Grid">прямоугольной матрице</abbr>, без заранее расставленных <abbr title="Key waypoint">точек маршрута</abbr>. Каждая из этих библиотек подходит под эти критерии.
...
| paths | 6588751 ns | 5158604 ns | 6114856 <abbr title="Наносекунды; 6114856 ns - это ~0.006 секунд или ~6.1 ms">ns</abbr> |
...
| grid | 996889 <abbr title="Значение в байтах; 996889 байт - это почти мегабайт!">B</abbr> | 551976 B | 740523 B |
...
* Матрица <abbr title="Какие клетки проходимы, а какие заблокированы">проходимости</abbr>
* Представление результата поиска пути
* [Priority queue](https://en.wikipedia.org/wiki/Priority_queue) для <abbr title="Очередь следующих клеток для рассмотрения">frontier</abbr>
* Ассоциативный массив для хранения посещённых клеток

## Матрица проходимости

Элементы матрицы будем называть <abbr title="Cell">клетками</abbr>. У каждой клетки есть координата внутри матрицы.
...
Grid map отображает рельеф местности с точки зрения алгоритма поиска пути. В самом простом виде каждая клетка хранит <abbr title="Enum">значение-перечисление</abbr> тайла.
...
Введём понятие <abbr title="Layer">слоя</abbr>. Слой позволяет интерпретировать значение клетки матрицы в соответствии с категорией перемещения. Так как значения в матрице имеют значения от 0 до 3, нам нужно отображать всего четыре ключа.
...
От вставляемого компилятором неявного <abbr title="Автоматически вставляемая проверка на вхождение индекса в диапазон корректных">bound check</abbr> можно избавиться:
...
В новых версиях Go всё больше аргументов передаются через регистры и текущий Go tip в [godbolt](https://godbolt.org/) генерирует одну инструкцию [MOVUPS](https://www.felixcloutier.com/x86/movups) для передачи GridPath.
...
А вот извлечение из бакетной очереди требует изобретательности. В самом базовом варианте вам нужно перебирать бакеты до тех пор, пока не найдётся непустой. <abbr title="Dread Lord">Лорд ужаса</abbr> из WarCraft III сказал бы: "Дурацкий план."
...
Осталось реализовать Reset. Самый прямолинейный способ - это <abbr title="Это терминология Go для операции типа std::vector::clear() в C++ и чем-то похож на ArrayList.clear() в Java">реслайсинг</abbr> каждого бакета. Память будет доступна для следующего использования, а настоящего зануления нам всё равно не нужно.
...
Это довольно скромное количество ключей, поэтому в голову приходит обычный <abbr title="Так в Go называются динамические массивы, типа std::vector и ArrayList. Если называть их массивами, вам сразу же объяснят, почему вы не правы">слайс</abbr>, где ключом будет прямой индекс. У этой структуры будет значительный минус - Reset потребует полного зануления слайса.
...
Понравилась статья? У нас в [сообществе разработки игр на Go](https://t.me/go_gamedev) всегда весело! Присоединяйтесь, будем создавать игрушки на нашем любимом языке программирования вместе.
